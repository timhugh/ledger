...
func testClient(t *testing.T) (*assert.Assertions, *Client, *sqlx.DB) {
	...
	client := &Client{db: db}
	assert.NoError(client.Migrate(context.Background()))
...
func TestCreateJournal(t *testing.T) {
	...
	t.Run("creates journals", func(t *testing.T) {
		...
		assert.NoError(client.CreateJournal(context.Background(), expectedJournal))
	...
	t.Run("automatically assigns IDs", func(t *testing.T) {
		...
		assert.NoError(client.CreateJournal(context.Background(), newJournal))
	...
	t.Run("does not allow duplicate uuids", func(t *testing.T) {
		...
		assert.NoError(client.CreateJournal(context.Background(), expectedJournal))
		assert.Error(client.CreateJournal(context.Background(), expectedJournal))
	})
	t.Run("requires a name", func(t *testing.T) {
		...
		err := client.CreateJournal(context.Background(), actualJournal)
		assert.Error(err)
	})
}
func TestGetJournal(t *testing.T) {
	...
	t.Run("returns a journal by id", func(t *testing.T) {
		...
		actualJournal, err := client.GetJournal(context.Background(), expectedJournal.JournalUUID)
		assert.NoError(err)
	...
	t.Run("returns an error if the journal does not exist", func(t *testing.T) {
		assert, client, _ := testClient(t)
		journal, err := client.GetJournal(context.Background(), "nonexistent")
		assert.Equal(ledger.ErrNotFound, err)
		assert.Nil(journal)
	})
...
func TestGetTransactions(t *testing.T) {
	...
	t.Run("returns transactions for a journal", func(t *testing.T) {
		...
		actualTransactions, err := client.GetTransactions(context.Background(), expectedJournal.JournalUUID)
		assert.NoError(err)
...
func TestGetSession(t *testing.T) {
	t.Run("satifies SessionGetter interface", func(t *testing.T) {
		assert, client, _ := testClient(t)
		_, ok := interface{}(client).(ledger.SessionGetter)
		assert.True(ok)
	})
	t.Run("returns a session by id", func(t *testing.T) {
		assert, client, db := testClient(t)

		_, err := db.Exec("INSERT INTO sessions (session_uuid, user_uuid) VALUES (?, ?)", "session-uuid", "user-uuid")
		assert.NoError(err)
		_, err = db.Exec("INSERT INTO users (user_uuid, login, password_hash, password_salt) VALUES (?, ?, ?, ?)", "user-uuid", "test", "hash", "salt")
		assert.NoError(err)

		session, err := client.GetSession(context.Background(), "session-uuid")
		assert.NoError(err)
		expectedSession := &ledger.Session{
		...
		assert.EqualValues(expectedSession, session)
	})
	t.Run("returns an error if the session does not exist", func(t *testing.T) {
		assert, client, _ := testClient(t)
		session, err := client.GetSession(context.Background(), "nonexistent")
		assert.Equal(ledger.ErrNotFound, err)
		assert.Nil(session)
	})
}
